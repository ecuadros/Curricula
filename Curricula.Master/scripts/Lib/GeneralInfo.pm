...
sub generate_table_of_courses_by_specific_outcome($)
{
	...
	my $version = $Common::config{OutcomesVersion};
	my ($pagesize, $halfpage) = ("23cm", "10.5cm");
	my $begin_table = "%". ("*"x50) ."\n";
	...
	my $one_line_template = "<<SpecificOutcomeLabel>> & <<SpecificOutcome>> & \n<<Courses>> \\\\ \\hline\n\n";
	my %map = ();
	my $counter = 0;
	foreach my $outcome (split(",", $Common::config{outcomes_list}{$version}))
	{
		...
		{
			$map{SpecificOutcomeLabel} = "$Common::config{specificoutcome}{$lang}{$outcome}{$number}{label})";
			$map{SpecificOutcome}  = "\\begin{minipage}{$halfpage}\n";
			$map{SpecificOutcome} .= "$Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt}\n";
			...
			{	$header = "\\multicolumn{3}{|l|}{\n";
				$header .= "\\begin{minipage}{$pagesize}\n";
