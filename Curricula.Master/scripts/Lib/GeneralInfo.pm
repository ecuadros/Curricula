...
sub generate_table_of_courses_by_specific_outcome($)
{
	...
	my $version = $Common::config{OutcomesVersion};
	$output_txt .= "\\begin{center}\n";
	$output_txt .= "\\begin{tabularx}{23cm}{|X|X|X|}\\\\ \\hline\n";
	...
	my $one_line_template = "<<Outcome>> & <<SpecificOutcome>> & \n <<Courses>> \\\\ \\hline\n\n";
	my %map = ();
	foreach my $outcome (split(",", $Common::config{outcomes_list}{$version}))
	{
		...
		{
			($map{SpecificOutcome} = "$Common::config{specificoutcome}{$lang}{$outcome}{$number}{label}) $Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt}") =~ s/\\xspace//g;
			...
			if($courses_counter > 0)
			{
				$map{Courses}  = "\\begin{minipage}{0.95\\textwidth}\n";
			...
			$output_txt .= Common::replace_tags_from_hash($one_line_template, "<<", ">>", %map);
		}
	}
	$output_txt .= "\\end{tabularx}\n";
	$output_txt .= "\end{center}\n";
